...
const listaCertificados = [
	{ id: 1, titulo: "Engenharia de Software - FIAP", img: "img/certificados/Engenharia do Software.png" },
	{ id: 2, titulo: "DevOps - FIAP", img: "img/certificados/Devops Fiap.png" },
	{ id: 3, titulo: "SQL/MYSQL - UDEMY", img: "img/certificados/SQL.jpg" },
	{ id: 4, titulo: "Neuro Marketing - ACADEMIA NEUROMARKETING", img: "img/certificados/Neuro Marketing.png" },
	{ id: 5, titulo: "Praticas Devops VSC - MICROSOFT", img: "img/certificados/Praticas Devops VSC - Microsoft.png" },
	{ id: 6, titulo: "Java - FIAP", img: "img/certificados/Java Fiap.png" },
	{ id: 7, titulo: "Introdução à Programação Orientada a Objetos (POO) - BRADESCO", img: "img/certificados/Introducao POO.png" },
];
...
export default function Home() {
	...
	return (
		<main className="min-h-screen bg-slate-50 font-sans text-gray-900 overflow-x-hidden scroll-smooth relative">
			...
			{/* MODAL DETALHADO: SOBRE MIM */}
			<AnimatePresence>
				{showAboutModal && (
					<motion.div
						initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
						onClick={() => setShowAboutModal(false)}
						className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-6 cursor-zoom-out"
					>
						<motion.div
							initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
							onClick={(e) => e.stopPropagation()}
							className="bg-white rounded-[56px] p-10 md:p-16 max-w-3xl w-full shadow-2xl relative max-h-[90vh] overflow-y-auto"
						>
							...
							<div className="space-y-8">
								<div>
									<h4 className="font-black text-blue-600 uppercase text-xs mb-3 tracking-widest">Resumo Profissional</h4>
									<p className="text-slate-600 text-lg leading-relaxed text-justify">{resumoProfissional}</p>
								</div>
								<div className="h-[1px] bg-slate-100 w-full"></div>
								<div>
									<h4 className="font-black text-blue-600 uppercase text-xs mb-3 tracking-widest">Vida Pessoal & Hobbies</h4>
									<p className="text-slate-600 text-lg leading-relaxed text-justify">{hobbiesDetalhado}</p>
								</div>
							</div>
						</motion.div>
					</motion.div>
			...
			<nav className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100 py-4 px-8 flex justify-between items-center uppercase font-black text-[10px] tracking-[0.2em]">
				<span className="text-blue-600 text-2xl tracking-tighter uppercase">LP.dev</span>
				<ul className="flex gap-8 items-center text-slate-400">
					...
					<li><a href="#formacao" className="hover:text-blue-600 transition-colors">Formação</a></li>
					<li><a href="#certificados" className="hover:text-blue-600 transition-colors">Certificados</a></li>
					<li><a href="#experiencia" className="hover:text-blue-600 transition-colors">Experiência</a></li>
			...
			<section id="sobre" className="pt-48 pb-24 px-6 max-w-6xl mx-auto md:text-left text-center">
				<div className="flex flex-col md:flex-row gap-16 items-center">
					...
					<div className="flex-1 overflow-hidden">
						<h1 className="text-2xl lg:text-4xl font-black tracking-tighter leading-none mb-6 uppercase whitespace-nowrap">
							Leonardo de Paula
						</h1>
						<p className="text-3xl text-blue-600 font-bold min-h-[40px] mb-8 tracking-tight italic uppercase">
							<Typewriter texts={["Full Stack Developer", "BI Specialist", "Curitiba/PR", `${idade} Anos`]} />
						</p>
						<p className="text-slate-400 text-xl font-medium leading-relaxed max-w-2xl text-justify mb-8">
							Atuo com tecnologia desde 2009. Especialista em resolver problemas complexos através do código com foco em Full Stack e BI.
						</p>
						<button
							onClick={() => setShowAboutModal(true)}
							className="bg-blue-600 text-white px-8 py-4 rounded-full font-black text-xs uppercase tracking-[0.2em] hover:bg-blue-700 transition-all hover:scale-105 shadow-xl shadow-blue-600/20"
						>
							Ver mais sobre mim
						</button>
					</div>
